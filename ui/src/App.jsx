...
const ProfileForm = ({ profile, onSave, onCancel }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (resumeFile && form.id) {
        await api.uploadResume(form.id, resumeFile);
      }
      onSave();
    } catch (e) {
  ...
  const Field = ({ label, field, type = "text", placeholder = "", half = false, required = false }) => (
  ...
  const SelectField = ({ label, field, options, half = false }) => (
    <div className={half ? "col-span-1" : "col-span-2"}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Email / LinkedIn Login" field="email" type="email" placeholder="you@email.com" required />
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="First Name" field="first_name" placeholder="John" half required />
              <Field label="Last Name" field="last_name" placeholder="Doe" half required />
              <Field label="Phone" field="phone" placeholder="+1 555 000 0000" half />
              <Field label="City" field="city" placeholder="San Francisco" half />
              <SelectField label="Country" field="country" half options={[
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <SelectField label="Experience Level" field="experience_level" half options={[
              ...
              <Field label="Years of Experience" field="years_experience" placeholder="2" half />
              <SelectField label="Education Level" field="education_level" half options={[
              ...
              <Field label="Expected Salary (Annual)" field="expected_salary" placeholder="80000" half />
              <Field label="GPA (optional)" field="gpa" placeholder="3.8" half />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="LinkedIn URL" field="linkedin_url" placeholder="linkedin.com/in/johndoe" />
              <Field label="GitHub" field="github" placeholder="github.com/johndoe" half />
              <Field label="Portfolio / Website" field="website" placeholder="johndoe.dev" half />
            </div>
          </div>
...
const SessionMonitor = ({ sessionId, onClose }) => {
  ...
  const logs = isDemo ? demoLogs : (session?.logs || []);
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={async () => {
                        if (confirm("Delete this profile?")) {
                          try { await api.deleteProfile(p.id); } catch { }
                          loadProfiles();
                        }
